...
export const Timeline = ({ data }: { data: TimelineEntry[] }) => {
  ...
  // Re-measure as content settles — project screenshots load after mount and
  // change the timeline's height, which a one-shot measurement would miss.
  useEffect(() => {
    ...
    >
      <div className="max-w-7xl mx-auto py-20 px-4 md:px-8 lg:px-10">
        ...
        <p className="text-muted-foreground text-sm md:text-base max-w-sm">
          Experience and projects that shaped my path as a developer.
        </p>
      </div>
        ...
        {data.map((item, index) => (
          ...
          >
            <div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-full">
              ...
              <h3 className="hidden md:block text-xl md:pl-20 md:text-5xl font-bold text-primary/60">
                {item.title}
              </h3>
            </div>
